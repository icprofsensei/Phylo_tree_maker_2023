...
'#440154']
                ...
                for key in weighteddict.keys():
                        ...
                        if 'species' in rankdict.values() and weighteddict[key] > 1.5 :
                                ...
                                tblabelled.append(key)
                        elif 'phylum' in rankdict.values():
                                tblabelled.append(key)
                           '''     
        ...
        def layoutfunc(self, node):
                ...
                '''
                  nohorline = False
                  node.img_style["fgcolor"] = "#96f4ad"
                  node.img_style["vt_line_color"] ="#96f4ad"
                  node.img_style["hz_line_color"] ="#96f4ad"
                  node.img_style["size"] =0
                  node.img_style["vt_line_width"] = 0
                  node.img_style["hz_line_width"] = 0
                  if node.name in tblabelled:
                          if node.name != '131567' and node.name !=  '2759' and node.name != '33154' and node.name != '1783270' and node.name != '68336' and node.name != '451864':
                                faces.add_face_to_node(TextFace(tblabellednames[int(node.name)], ftype = 'arial', fsize = 7, fgcolor = "000000", penwidth=0, fstyle= 'normal', tight_text = False, bold = False), node, column = 1, position = "float")
                          '''
                  if node.name in colourdict.keys():
                        ...
                        node.img_style["vt_line_width"] = int(amplifier * 0.18)
                        node.img_style["hz_line_width"] = int(amplifier * 0.18)
